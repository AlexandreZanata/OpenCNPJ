...
func TestResponseCacheSetsShortPrivateOnCNPJMiss(t *testing.T) {
	app := fiber.New()
	app.Use(ResponseCache())
	app.Get("/api/v1/cnpj/:cnpj", func(c *fiber.Ctx) error {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{"error": "cnpj_not_found"})
	})

	req := httptest.NewRequest(fiber.MethodGet, "/api/v1/cnpj/37511144000112", http.NoBody)
	resp, err := app.Test(req)
	if err != nil {
		t.Fatal(err)
	}
